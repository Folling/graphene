...
#[derive(Debug)]
pub struct ShaderInner<const S: ShaderType> {
    /// The id of the shader, generated by OpenGL and valid for the lifetime of the shader
    pub id: gl::types::GLuint,
}

impl<const S: ShaderType> PartialEq for ShaderInner<S> {
    fn eq(&self, other: &Self) -> bool {
        self.id.eq(&other.id)
    }
}

impl<const S: ShaderType> Eq for ShaderInner<S> {}

impl<const S: ShaderType> std::hash::Hash for ShaderInner<S> {
...
impl<const S: ShaderType> PartialOrd for ShaderInner<S> {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

impl<const S: ShaderType> Ord for ShaderInner<S> {
...
pub struct Shader<const S: ShaderType> {
    inner: ShaderInner<S>,
}
...
impl<const S: ShaderType> Shader<S> {
    ...
    pub fn new() -> Result<Shader<S>, ShaderCreationError> {
...
impl<const S: ShaderType> Shader<S> {
    ...
    pub fn compile<S2: AsRef<str>>(self, src: S2) -> Result<CompiledShader<S>, ShaderCompileError> {
...
#[derive(Debug)]
pub struct CompiledShaderInner<const S: ShaderType> {
...
*/
#[derive(Debug)]
pub struct CompiledShader<const S: ShaderType> {
    inner: CompiledShaderInner<S>,
}

impl<const S: ShaderType> CompiledShader<S> {
    /// Returns a reference to the inner (private) data of the shader.
    /// Use at your own risk, no guarantees are made to the data itself.
    pub unsafe fn inner(&self) -> &CompiledShaderInner<S> {
        &self.inner
    }

    /// Returns a reference to the inner (private) data of the shader.
    /// Use at your own risk, no guarantees are made to the data itself, mutating it is to be considered UB.
    pub unsafe fn inner_mut(&mut self) -> &mut CompiledShaderInner<S> {
        &mut self.inner
    }
